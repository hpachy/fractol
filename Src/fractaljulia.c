...
void		init_fractol_julia(t_data *d)
{
	d->f->z_r = ((double)d->f->x - (650) - (d->f->x_tmp / ((1300
		/ (d->f->zoom / 1.1)) / 1300))) / (d->f->zoom) + d->f->x1;
	d->f->z_i = ((double)d->f->y - (650) - (d->f->y_tmp / ((1300
		/ (d->f->zoom / 1.1)) / 1300))) / (d->f->zoom) + d->f->y1;
	d->f->z_r += (1.3) + d->f->x_tmp;
	d->f->z_i += (1.3) + d->f->y_tmp;
	d->f->i = 0;
	d->f->verif = 0;
}
...
void		algo_julia(t_data *d)
{
	...
	while (++d->f->y < d->f->image_y)
	{
		...
		{
			init_fractol_julia(d);
			while ((d->f->verif < 4) && d->f->i < d->f->iteration_m)
			{
				d->f->tmp = d->f->z_r;
				d->f->z_r = d->f->z_r * d->f->z_r -
				d->f->z_i * d->f->z_i + d->f->c_r;
				d->f->z_i = 2 * d->f->z_i * d->f->tmp + d->f->c_i;
				d->f->i++;
